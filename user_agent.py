...
class TestUserAgent:
    ...
    def test_user_agent(self, values):
        ...
        obj = response.json()
        assert "platform" in obj, "There is no platform in the response"
        assert "browser" in obj, "There is no browser in the response"
        assert "device" in obj, "There is no device in the response"

        assert obj == values, "Result does not match expected value"